...
from app.schemas.match_words import MatchWordIn
from app.services import match_words_service as service

router = APIRouter(prefix="/admin/match-words", tags=["admin-match-words"])
...
@router.post("/entries")
def create_entry(payload: MatchWordIn, telegram_id: int, db: Session = Depends(get_db)):
    require_admin(telegram_id)
...
def update_entry(entry_id: int, payload: MatchWordIn, telegram_id: int, db: Session = Depends(get_db)):
    require_admin(telegram_id)
    entry = service.update_entry(db, entry_id, payload)
    if not entry:
        raise HTTPException(status_code=404, detail="match_word_not_found")
    return {"ok": True, "entry": service.serialize_entry(entry)}
...
def activate_entry(entry_id: int, telegram_id: int, db: Session = Depends(get_db)):
    require_admin(telegram_id)
    entry = service.set_active(db, entry_id, True)
    if not entry:
        raise HTTPException(status_code=404, detail="match_word_not_found")
    return {"ok": True, "entry": service.serialize_entry(entry)}
...
def deactivate_entry(entry_id: int, telegram_id: int, db: Session = Depends(get_db)):
    require_admin(telegram_id)
    entry = service.set_active(db, entry_id, False)
    if not entry:
        raise HTTPException(status_code=404, detail="match_word_not_found")
    return {"ok": True, "entry": service.serialize_entry(entry)}
