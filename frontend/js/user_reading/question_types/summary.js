...
UserReading.isSummaryQuestion = function (question) {
  return String(question?.type || "").toUpperCase() === "TEXT_INPUT"
    && question?.meta?.mode === "summary";
};

UserReading.groupSummaryQuestions = function (items) {
  const result = [];

  items.forEach((item) => {
    if (UserReading.isSummaryQuestion(item) && item.question_group_id) {
...
UserReading.renderSummaryBlankControl = function (question, number, wordBank) {
    ...
    return `
      <span class="summary-inline-blank">
        ...
        <select name="q_${question.id}" class="summary-blank-select">
          ...
          ${wordBank.map((word) => `
            <option value="${UserReading.escapeHtml(word)}">
              ${UserReading.escapeHtml(word)}
            </option>
          `).join("")}
        </select>
      </span>
...
UserReading.renderSummaryGroup = function (group, startNumber) {
  ...
  parts.forEach((part, index) => {
    summaryHtml += `<span>${UserReading.escapeHtml(part)}</span>`;

    if (index < blankCount && group.questions[blanksUsed]) {
      summaryHtml += UserReading.renderSummaryBlankControl(
      ...
      ${wordBank.length ? `
        <div class="summary-word-bank">
          ...
          <div class="summary-word-bank-items">
            ${wordBank.map((word) => `
              <span class="summary-word-bank-item">${UserReading.escapeHtml(word)}</span>
            `).join("")}
          </div>
        </div>
      ` : ""}
      ...
      ${remaining.length ? `
        <div class="summary-extra-blanks">
          ${remaining.map((question, index) =>
            UserReading.renderSummaryBlankControl(
              question,
