...
UserListening.initResultActions = function (data) {
  ...
  if (shareBtn) shareBtn.onclick = () => UserListening.shareResult(data);
  if (storyBtn) storyBtn.onclick = () => UserListening.shareStoryResult(data);
  if (homeLink) homeLink.onclick = () => (goBackToProfile ? goProfile() : goHome());
};

UserListening.shareStoryResult = async function ({ band, correct, total }) {
  ...
  try {
    ...
    const blob = await UserListening.resultCardToBlob({
    ...
    const uploaded = await UserListening.uploadResultCardImage(blob);
...
UserListening.shareResult = function ({ band, correct, total }) {
...
UserListening.resultCardToBlob = function ({ band, correct, total, dateText }) {
...
UserListening.blobToDataUrl = function (blob) {
  return new Promise((resolve, reject) => {
...
UserListening.uploadResultCardImage = async function (blob) {
  const imageBase64 = await UserListening.blobToDataUrl(blob);
...
UserListening.showResultBadge = function (text = "Saved!", type = "success") {
